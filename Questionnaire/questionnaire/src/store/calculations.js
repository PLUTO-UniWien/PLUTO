...
export default {
  ...
  calculateNormalizedAxisScore(data){
    ...
    let normalizeXminus = d3.scaleLinear()
    .domain([-51,0])
    .range([-1,0]);

    let normalizeXplus = d3.scaleLinear()
    .domain([0,23])
    .range([0,1]);

    let normalizeYminus = d3.scaleLinear()
    .domain([-9,0])
    .range([-1,0]);

    let normalizeYplus = d3.scaleLinear()
    .domain([0,17])
    .range([0,1]);
    ...
    let normalized = {x: score.x < 0 ? normalizeXminus(score.x) : normalizeXplus(score.x), y: score.y < 0 ? normalizeYminus(score.y) : normalizeYplus(score.y)}
