...
<div>
  <div class="background">
    ...
    <b-row style="padding-bottom: 60px; padding-top: 40px">
      <b-col>
        <div ref="chartarea" id="chartArea">
          <svg id="axisResultSvg" width="300" style="border-radius: 10px; margin: 10px; box-shadow: 3px 3px 6px #cccccc;">
    ...
    <div v-if="showScores">
      ...
      x: {{this.normalizedAxisScore.x}}, y: {{this.normalizedAxisScore.y}}
      <br> <br>
    </div>
    ...
    <div v-if="showTooltips">
      <b-tooltip target="highPublicValue" triggers="hover" style="width:300px" class="feedbackTooltip">
        <b>You would have received a higher score: </b><br><br>
        <ul>
          <li v-for="(point, index) in feedback.value" :key="point + index">{{point}}</li>
        </ul>
      </b-tooltip>
      ...
      <b-tooltip target="lowRisk" triggers="hover" class="feedbackTooltip">
        <b>You would have received a higher score: </b><br><br>
        <ul>
          <li v-for="(point, index) in feedback.risk" :key="point + index">{{point}}</li>
        </ul>
      </b-tooltip>
...
<b-row style="background-color: #ffffff">
  <div id="feedback">
    ...
    <p> The conception of public value underpinning this tool focuses on the sustainability of benefits, the extent to which these benefits address future societal needs, the distribution of benefits, the nature and severity of (conceivable) harm,
      the stability of safeguards, that is routine risk assessment prior to and during the deployment of the data use, and the possibility of correcting occurred harm swiftly and effectively. The public value is a weighted composite of risks and
      benefits of the data use. </p>
    ...
    <p>If benefits are high, but so are risks, this means that the data use that you inquired about creates some public value. It is likely to benefit people and communities but also puts some people at risk. In this case, the data use should only go
      ahead if the risks can be reduced to an acceptable level. </p>

    <h4>Low Public Value</h4>
    <p>If benefits are low, and so are risks, the data use in question creates low public value. Nevertheless, because the associated risks are low, it may go ahead if rules and practices are in place that ensure that some of the commercial profits
      obtained with the data come back to people and communities. You can find <a href="https://www.thelancet.com/journals/landig/article/PIIS2589-7500(22)00189-3/fulltext">more information on benefit sharing here.</a> </p>

    <h4>No Public Value</h4>
    <p>If benefits are low and risks are high, the data use in question creates no public value. Such data use should not go ahead. </p>

    <h3 style="color: #3e85c7">Feedback </h3>
    <p>If you feel that this assessment is unfair and misses an important type of value that would be created by the data use, <a href="mailto:seliem.el-sayed@univie.ac.at"> please let us know. </a></p>
  </div>
</b-row>
...
export default {
  ...
  methods: {
    drawChart() {
      ...
      let windowSize = 500
      try {
        windowSize = this.$refs.chartarea.clientWidth < 500 ? this.$refs.chartarea.clientWidth : 500
      } catch {
        windowSize = 500
      }
      ...
      axisX.scale(scaleX)
      d3.select("#chartElements").append("g").call(axisX)
        .attr("transform", "translate(0," + (chartsize / 2) + ")")
        .attr("class", "axis")

      axisY.scale(scaleY)
      d3.select("#chartElements").append("g").call(axisY)
      ...
      d3.select("#resultPoint")
        ...
        .attr("id", "resultPoint")
        .on("mouseover", () => {
          d3.select("#resultPoint").transition().duration(100).attr("r", radius + 3)
        })
        .on("mouseout", () => {
