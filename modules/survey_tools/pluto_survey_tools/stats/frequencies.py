...
def question_choice_sum_expanded(question: model.Question) -> list[tuple[float, float]]:
    ...
    base = [0] * len(key_idx)
    for s in all_possible_sums_gen(
        question.selection_range.end, question.choice_scores
    ):
...
def score_count_freq(questions: list[model.Question]) -> Counter:
    ...
    """
    score_counters = [
        Counter(all_possible_sums_gen(q.selection_range.end, q.choice_scores))
        for q in questions
    ]
...
def score_count_df(
    questions: list[model.Question], normalize: bool = False
) -> pd.DataFrame:
    """
    Return a DataFrame of the possible scores and their frequencies.
    :param questions: A list of questions.
