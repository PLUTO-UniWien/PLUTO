import type { APIResponseData, StrapiType } from "@/modules/strapi/types";
import { strapiPublicUrl } from "@/modules/strapi/utils";

type Survey = StrapiType<"api::survey.survey">;
type Question = StrapiType<"api::question.question">;
type AnswerChoice = StrapiType<"question.choice">;
 ...
 */
export function generateSurveyJsModelJSON(survey: Survey) {
 ...
 *
 * @param {Survey} survey - The survey to generate core properties for.
 */
function createCoreProperties(survey: Survey) {
  return {
    title: survey.title,
 ...
 *
 * @param {Survey} survey - The survey containing the questions.
 */
function prepareQuestionsWithPageTitles(survey: Survey) {
 ...
 */
export function answerChoiceLabel(questionNumber: number, choiceNumber: number) {
...
export function answerChoiceLabelInverse(label: string) {
  ...
  const questionNumber = Number.parseInt(questionNumberString.slice(1), 10);
  const choiceNumber = Number.parseInt(choiceNumberString, 10);
...
export function questionLabelInverse(label: string) {
  const questionNumber = Number.parseInt(label.slice(1), 10);
...
function mapQuestion(question: Question, questionNumber: number) {
  ...
  const choices = question.choices
    ...
    .map((choice, choiceNumber) => mapAnswerChoice(choice, questionNumber, choiceNumber + 1));
